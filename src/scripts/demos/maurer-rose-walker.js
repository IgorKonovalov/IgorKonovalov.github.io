...
const inputN = document.getElementById('walker-n');
const startB = document.getElementById('walker-start');
...
function drawRose(n) {
  ...
  for (let a = 0; a < 360; a += 2) {
    ...
    const r = Math.sin(n * deg) * (xStart - 20);
    const x = xStart + r * Math.cos(deg);
    const y = xStart + r * Math.sin(deg);
    cx.lineTo(x, y);
  }
  cx.stroke();
}
