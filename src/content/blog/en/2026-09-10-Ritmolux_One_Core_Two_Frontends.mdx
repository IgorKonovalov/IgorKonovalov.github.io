...
One detail about the pictures, including the four in this post. **Every image in that repository is a headless render of the engine**, captured by the `shot` CLI under a synthesized audio clip — not a screenshot of the application window. They are regenerated by a script whose manifest records the preset, stimulus, hop, size and tier behind each one. There is no picture anywhere of the preset browser, the settings menu or the diagnostics overlay.
